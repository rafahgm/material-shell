    ...
    PanelWindow {
        ...
        readonly property string colorSourcePath: {
            if (wallpaperIsVideo) {
                ...
                const ff = WallpapersService.getVideoFirstFramePath(wallpaperPathRaw)
                // Cache-bust so ColorQuantizer reloads when the first frame appears.
                if (ff) return ff + "?ff=1"
                WallpapersService.ensureVideoFirstFrame(wallpaperPathRaw)
                return WallpapersService._videoThumbDir + "/" + MD5.hash(wallpaperPathRaw) + ".jpg?ff=0"
            }
