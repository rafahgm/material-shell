...
Singleton {
	...
	readonly property bool isYtMusicActive: {
		if (YtMusicService.currentVideoId) return true;
		if (YtMusicService.mpvPlayer) return true;
	...
	function isRealPlayer(player) {
		...
		if (name === "org.mpris.MediaPlayer2.mpv" || name.startsWith("org.mpris.MediaPlayer2.mpv.instance")) {
			if (YtMusicService.mpvPlayer) return player === YtMusicService.mpvPlayer;
	...
	function _isYtMusicMpv(player): bool {
		...
		if (YtMusicService.mpvPlayer && player === YtMusicService.mpvPlayer) return true;
		...
		if (YtMusicService.currentVideoId || YtMusicService.currentTitle) {
			const ytTitle = _normTitle(YtMusicService.currentTitle);
	...
	function _isYtMusicRelated(player): bool {
		...
		if (!YtMusicService.currentVideoId && !YtMusicService.currentTitle) return false;
		const trackUrl = player.metadata?.["xesam:url"] ?? "";
	...
	function _filterYtMusicDuplicates(playerList) {
		...
		if (ytMusic.length > 1) {
			...
			let chosen = ytMusic.find(p => YtMusicService.mpvPlayer && p === YtMusicService.mpvPlayer);
	...
	function togglePlaying(): void {
		if (root.isYtMusicActive && YtMusicService.currentVideoId) {
			YtMusicService.togglePlaying();
