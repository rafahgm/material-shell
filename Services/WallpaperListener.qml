...
Singleton {
    ...
    function wallpaperUrlForScreen(screen): string {
        if (multiMonitorEnabled && screen) {
            ...
            if (data && data.path) {
                ...
                if (isVideoPath(p)) {
                    const ff = WallpapersService.getVideoFirstFramePath(p);
                    if (ff)
                        return (ff.startsWith("file://") ? ff : "file://" + ff) + "?ff=1";
                    const expected = WallpapersService._videoThumbDir + "/" + MD5.hash(p) + ".jpg";
                    WallpapersService.ensureVideoFirstFrame(p);
                    return "file://" + expected + "?ff=0";
                }
                return p.startsWith("file://") ? p : "file://" + p;
            }
        }
        return WallpapersService.effectiveWallpaperUrl;
    }
    ...
    function refresh() {
        ...
        if (JSON.stringify(effectivePerMonitor) === newStr)
            return;
