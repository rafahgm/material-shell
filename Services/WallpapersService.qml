...
Singleton {
    ...
    readonly property bool _debugWallpaperUrls: (Quickshell.env("MATERIAL_SHELL_DEBUG_WALLPAPER_URLS") ?? "") === "1"

    // Wallpaper path resolution for aurora/backdrop
    readonly property bool isWindowsFamily: (Config.options?.panelFamily ?? "linux") === "windows"
    readonly property bool useBackdropWallpaper: isWindowsFamily
        ? (Config.options?.windows?.background?.backdrop?.hideWallpaper ?? false)
    ...
    Process {
        ...
        environment: ({
            "MATERIAL_SHELL_VIRTUAL_ENV": Quickshell.env("HOME") + "/.local/state/quickshell/.venv"
        })
        stdout: SplitParser {
            onRead: data => {
