...
Singleton {
    ...
    function refineData(apiData) {
        ...
        root.data = result;
        console.info("[WeatherService] Updated:", result.temp, result.city);
    }
    ...
    function refineOpenMeteoData(apiData): void {
        ...
        root.data = result
        console.info("[WeatherService] Updated via Open-Meteo:", result.temp, result.city)
    }
    ...
    function resolveLocation(): void {
        ...
        if (root.hasManualCoords) {
            ...
            console.info("[WeatherService] Using manual coordinates:", root.configLat, root.configLon);
            root.location = {
                valid: true,
                lat: root.configLat,
                lon: root.configLon,
        ...
        if (root.hasManualCity) {
            ...
            console.info("[WeatherService] Using manual city:", root.configCity);
        ...
        if (root.enableGPS) {
            console.info("[WeatherService] Trying GPS via geoclue...");
    ...
    function getLocation(): void {
        ...
        console.info("[WeatherService] Getting location from IP...");
    ...
    function forceRefresh(): void {
        console.info("[WeatherService] Force refresh requested");
    ...
    Timer {
        ...
        onTriggered: {
            if (root._retryCount < 5) {
                root._retryCount++;
                console.info("[WeatherService] Retry attempt", root._retryCount);
                if (!root.location.valid) {
    ...
    Process {
        ...
        stdout: StdioCollector {
            onStreamFinished: {
                if (text.length === 0) {
                    console.warn("[WeatherService] Forward geocode empty, falling back to city name");
                ...
                try {
                    ...
                    if (Array.isArray(results) && results.length > 0) {
                        ...
                        console.info("[WeatherService] Geocoded:", root.configCity, "→", displayName, "(", lat, ",", lon, ")");
                        root.fetchWeather();
                    } else {
                        console.warn("[WeatherService] No geocode results for:", root.configCity);
                        root.location = { valid: true, lat: 0, lon: 0, name: root.configCity };
                        root.fetchWeather();
                    }
                } catch (e) {
                    console.error("[WeatherService] Geocode parse error:", e.message);
                    root.location = { valid: true, lat: 0, lon: 0, name: root.configCity };
                    root.fetchWeather();
                }
            }
                ...
                try {
                    ...
                    if (addr) {
                        ...
                        if (name) {
                            ...
                            console.info("[WeatherService] Reverse geocoded:", name);
                        }
                    }
                } catch (e) {
                    console.warn("[WeatherService] Reverse geocode error:", e.message);
                }
                root.fetchWeather();
            }
        }
    ...
    Process {
        ...
        stdout: StdioCollector {
            onStreamFinished: {
                if (text.trim().length === 0) {
                    console.warn("[WeatherService] GPS failed, falling back to IP");
                ...
                if (parts.length >= 2) {
                    ...
                    if (!isNaN(lat) && !isNaN(lon)) {
                        ...
                        console.info("[WeatherService] GPS location:", lat, lon);
                ...
                console.warn("[WeatherService] GPS parse failed, falling back to IP");
                gpsLocator._handledFallback = true;
                root.getLocation();
            }
        }
        onExited: (code) => {
            if (code !== 0 && !root.location.valid && !gpsLocator._handledFallback) {
                console.warn("[WeatherService] GPS process failed (code " + code + "), falling back to IP");
                gpsLocator._handledFallback = true;
                root.getLocation();
            }
        }
    ...
    Process {
        ...
        stdout: StdioCollector {
            onStreamFinished: {
                if (text.length === 0) {
                    console.warn("[WeatherService] IP location empty, trying fallback");
        ...
        onExited: (code) => {
            if (code !== 0) {
                console.warn("[WeatherService] IP location failed, trying fallback");
                fallbackLocator.running = true;
            }
        }
    }
            ...
            onStreamFinished: {
                ...
                try {
                    ...
                    if (data.latitude && data.longitude) {
                        ...
                        console.info("[WeatherService] Location (fallback):", root.location.name);
                        root.fetchWeather();
                    } else {
                    ...
                    console.error("[WeatherService] Fallback location error:", e.message);
                    retryTimer.start();
                }
            }
        }
    ...
    Process {
            ...
            onStreamFinished: {
                ...
                if (payload.length === 0) {
                    ...
                    if (root._emptyResponseCount >= 3) {
                        console.warn("[WeatherService] Empty response (x" + root._emptyResponseCount + "), retrying");
                    } else {
                        console.info("[WeatherService] Empty response, retrying");
                    }
                ...
                if (!(payload.startsWith("{") || payload.startsWith("["))) {
                    root._emptyResponseCount++;
                    if (root._emptyResponseCount >= 3) {
                        console.warn("[WeatherService] Non-JSON weather response, retrying");
                    } else {
                        console.info("[WeatherService] Transient weather response, retrying");
                    }
                    if (!fetcher._fallbackTriggered) {
                        fetcher._fallbackTriggered = true;
                        root._primaryFailCount++;
                ...
                try {
                    ...
                    root._emptyResponseCount++;
                    if (root._emptyResponseCount >= 3) {
                        console.warn("[WeatherService] Parse error:", e.message);
                    } else {
                        console.info("[WeatherService] Parse error, retrying");
                    }
                    if (!fetcher._fallbackTriggered) {
                        fetcher._fallbackTriggered = true;
                        root._primaryFailCount++;
        ...
        onExited: (code) => {
            if (code !== 0 && !fetcher._fallbackTriggered) {
                ...
                root._primaryFailCount++;
                root._primaryFailUntil = Date.now() + 30 * 60 * 1000;
                console.warn("[WeatherService] Primary provider failed, switching fallback. code:", code);
                root.fetchWeatherFallback();
            }
        }
    }
    ...
    Process {
        ...
        stdout: StdioCollector {
            onStreamFinished: {
                ...
                try {
                    ...
                    console.warn("[WeatherService] Open-Meteo parse error:", e.message)
                    retryTimer.start()
                }
            }
        }
        onExited: (code) => {
            if (code !== 0) {
                console.warn("[WeatherService] Open-Meteo fetch failed, code:", code)
                retryTimer.start()
            }
        }
    }
