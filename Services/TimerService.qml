...
Singleton {
    ...
    function refreshPomodoro() {
        ...
        if (getCurrentTimeInSeconds() >= Persistent.states.timer.pomodoro.start + pomodoroLapDuration) {
            ...
            if (Persistent.states.timer.pomodoro.isBreak && (pomodoroCycle + 1 == cyclesBeforeLongBreak)) {
                notificationMessage = TranslationService.tr(`🌿 Long break: %1 minutes`).arg(Math.floor(longBreakTime / 60));
            } else if (Persistent.states.timer.pomodoro.isBreak) {
                notificationMessage = TranslationService.tr(`☕ Break: %1 minutes`).arg(Math.floor(breakTime / 60));
            } else {
                notificationMessage = TranslationService.tr(`🔴 Focus: %1 minutes`).arg(Math.floor(focusTime / 60));
            }
    ...
    function refreshCountdown() {
        ...
        if (countdownSecondsLeft <= 0 && countdownRunning) {
            ...
            Quickshell.execDetached(["/usr/bin/notify-send", "Timer", TranslationService.tr("Time's up!"), "-a", "Shell", "-i", "alarm-symbolic"]);
