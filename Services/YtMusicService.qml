...
Singleton {
    ...
    function _tryNextBrowser(): void {
        ...
        if (root._quickConnectIndex >= root._browsersToTry.length) {
            ...
            root.googleError = TranslationService.tr("Could not connect. Log in to music.youtube.com in your browser first.")
            return
        }
...
`]
    ...
    function _checkGoogleConnection(): void {
        if (!root.available) {
            root.googleError = TranslationService.tr("yt-dlp not available")
            root.googleChecking = false
            return
        }
        root.googleChecking = true
        ...
        onExited: (code) => {
            ...
            if (code === 0 && stdOutput.trim().length > 0) {
                ...
                if (err.includes("sign in") || err.includes("403") || err.includes("not found")) {
                    ...
                    root.googleError = TranslationService.tr("Could not read cookies. Close %1 and try again.").arg(root.getBrowserDisplayName(root.googleBrowser))
                } else if (err.includes("network") || err.includes("connection") || err.includes("unable to download")) {
                    root.googleError = TranslationService.tr("Network error. Check your internet connection.")
                } else {
                    root.googleError = TranslationService.tr("Could not connect. Log in to music.youtube.com in your browser first.")
                }
            }
    ...
    Process {
        ...
        onExited: (code) => {
            if (code !== 0 && root.searchResults.length === 0) {
                root.error = TranslationService.tr("Search failed. Check your connection.")
            }
        }
    }
    ...
    Process {
        ...
        onExited: (code) => {
            ...
            if (code !== 0 && code !== 4 && code !== 9 && code !== 15) {
                root.error = TranslationService.tr("Playback failed")
            }
        }
    }
