...
Variants {
    ...
    PanelWindow {
        ...
        property bool wallpaperSafetyTriggered: {
            ...
            const sensitiveNetwork = (CF.StringUtils.stringListContainsSubstring(NetworkService.networkName.toLowerCase(), networkKeywords));
        ...
        property bool hasWindowsOnCurrentWorkspace: {
            try {
                if (NiriService.windows && NiriService.workspaces) {
        ...
        Item {
            ...
            Item {
                ...
                property real valueX: {
                    ...
                    if ((bgRoot.parallaxOptions.enableWorkspace ?? false) && !bgRoot.verticalParallax) {
                        const wsId = NiriService.focusedWorkspaceIndex ?? 1;
            ...
            ContextMenu {
                ...
                model: [
                    { text: TranslationService.tr("Settings"), iconName: "settings", monochromeIcon: true,
                        action: () => { Quickshell.execDetached(["/usr/bin/qs", "-c", "ii", "ipc", "call", "settings", "open"]) } },
                    { type: "separator" },
                    { text: TranslationService.tr("Change wallpaper"), iconName: "image", monochromeIcon: true,
                        action: () => { GlobalStates.wallpaperSelectorOpen = true } },
                    { text: TranslationService.tr("Screenshot"), iconName: "screenshot_monitor", monochromeIcon: true,
                        action: () => { GlobalStates.regionSelectorOpen = true } },
                    { type: "separator" },
                    { text: TranslationService.tr("Reload shell"), iconName: "refresh", monochromeIcon: true,
