...
QUICKSHELL_CONFIG_NAME="material-shell"
...
SHELL_CONFIG_FILE="$XDG_CONFIG_HOME/material-shell/config.json"
...
switch() {
    ...
    if [[ -n "${MATERIAL_SHELL_VIRTUAL_ENV:-}" ]]; then
        _ii_venv="$(eval echo "$MATERIAL_SHELL_VIRTUAL_ENV")"
    else
...
main() {
    ...
    detect_scheme_type_from_image() {
        ...
        local _det_venv
        if [[ -n "${MATERIAL_SHELL_VIRTUAL_ENV:-}" ]]; then
            _det_venv="$(eval echo "$MATERIAL_SHELL_VIRTUAL_ENV")"
        else
